...
build_musl_armhf() {
    ...
    # echo "GCC_BUILTIN_PREREQS=yes" >> config.sh
    echo "GCC_BOOTSTRAP_CONFFLAGS='--with-arch=armv7-a --with-float=hard --with-fpu=vfpv3-d16'" >> config.mak
    echo "GCC_CONFFLAGS='--with-arch=armv7-a --with-float=hard --with-fpu=vfpv3-d16'" >> config.mak
    make TARGET=arm-linux-musleabihf OUTPUT=/opt/cross/arm-linux-musleabihf install
...
build_nmap_armhf() {
    ...
    # make zlibstatic
    cd /build/nmap
    CC='/opt/cross/arm-linux-musleabihf/bin/arm-linux-musleabihf-gcc' \
        CXX='/opt/cross/arm-linux-musleabihf/bin/arm-linux-musleabihf-g++' \
